...
import { WRITINGS, type Calligraphy, type ProjectBlock } from "../lib/content";
...
const T = {
  zh: {
    ...
    labels: { about: "关于", beyond: "专业之外", work: "实习经历", projects: "其他项目经历", awards: "获奖", courses: "部分课程成绩", self: "自我评价", docs: "证明材料", contact: "联系", education: "教育" },
  ...
  en: {
    ...
    labels: { about: "About", beyond: "Beyond the Major", work: "Internships", projects: "Other Projects", awards: "Awards", courses: "Selected Coursework", self: "Self-Assessment", docs: "Documents", contact: "Contact", education: "Education" },
...
const WallBody = memo(function WallBody({ body, onZoom }: { body?: ProjectBlock[]; onZoom: (src: string) => void }) {
  return <div className="fb-poster-body">{renderBody(body, { onZoom, eager: true }).blocks}</div>;
});
            ...
            {WALL_ITEMS.map((p, i) => {
              ...
              <Fragment key={p.group + p.no}>
                ...
                <div className="fb-poster-collapse" style={{
                    maxHeight: `calc(72vh + ${pr * detailsH}px)`,
                  }}>
                 ...
                 linear-gradient(160deg, ${mixHex(base.white, p.accent, 0.18)}, ${mixHex(base.card, p.accent, 0.34)})`,
                  ...
                  {p.calli ? (
                    <CalligraphyArt image={p.calli.image || undefined} cropFrom={p.calli.cropFrom} cropTo={p.calli.cropTo}
                      zoomImage={p.calli.zoomImage} focus={p.calli.focus} chars={p.calli.chars} pr={pr} />
                  ) : (
                    <>
                      ...
                      <span className="fb-poster-art-mark" style={{ color: mixHex("#9a9a9a", p.accent, 0.45), opacity: pr < 0.5 ? 1 : (pr - 0.5) * 2, maxWidth: `${100 - pr * 73}%` }}>{t.pendingMark}</span>
                    </>
                  )}
                </div>
                <div className={"fb-poster-meta" + (pr < 0.5 ? " is-over" : "")} style={(() => {
              ...
              {shownIdx === i && (
                <div className="fb-poster-details" style={{
                  ...
                  {p.pending ? (
                    ...
                    <WallBody body={p.fullBody} onZoom={setWallZoom} />
                  )}
                </div>
              )}
                </div>
          ...
          <div className="fb-col">
            ...
            {!isOpen("edu") && (
              <div className="fb-compact">
                {EDUCATION[lang].map((e, ei) => <CompactRow key={e.title} title={e.title} note={e.sub} period={e.period} vt={!isOpen("edu") ? `vt-edu-${ei}` : undefined} />)}
              </div>
            )}
            <Reveal open={isOpen("edu")}>
              <div className="fb-list">
                {EDUCATION[lang].map((e, ei) => <EntryRow key={e.title} {...e} vt={isOpen("edu") ? `vt-edu-${ei}` : undefined} />)}
              </div>
            </Reveal>
            ...
            <h3 className="fb-sec"><span className="fb-sec-no">§ 9</span>{t.labels.docs}</h3>
            ...
            {/* § 10 联系（2026-09-13 白纸指令：Scott 式纯文字段，「这里」跳联系页 ?contact=1
                （页内=小红书+邮箱）；文案为白纸亲拟句） */}
            <h3 className="fb-sec"><span className="fb-sec-no">§ 10</span>{t.labels.contact}</h3>
            <div className="fb-prose">
              <p>
                {lang === "zh"
