...
export function FlipbookDemo() {
  ...
  useHeaderHeight();
  /* 断点跟随（2026-09-21）：手机上点海报=直接进文章页（不再墙内展开）——
     手机海报只有 ~176px 高，展开态的面板/页码胶囊/全文挤在一起必然错乱（白纸截图）；
     墙内展开读全文是桌面交互，手机上同样的内容由文章页承担（横幅取景已与海报一致）。 */
  ...
  /* 加载屏不再自动进书（2026-09-20 白纸指令：点按才进入）。原先此处有 4s 兜底墙钟
     （防旧内核定时器冻结），它会让"点按进入"在正常浏览器上也失效，已删；
     键盘用户用 Enter/Space（TrestleLoader 上的 role=button）。 */
  ...
  const WALL_ITEMS = [
    ...
    /* 文章在前（白纸 09-13：站上先只放自己的文章，作品暂不入列）；
       英文模式取各篇 en 块的标题/摘要/标签/正文，封面与日期不翻（文眼字是汉字） */
    ...WRITINGS.map((w, i) => {
      const en = w.en;
      return {
        group: "writing" as const,
        no: String(i + 1).padStart(2, "0"),
        title: (lang === "en" ? en?.title : undefined) || w.title,
        date: w.date ?? "",
        ...
        desc: { zh: w.summary ?? "", en: (en?.summary ?? w.summary) ?? "" },
        body: { zh: firstParagraphs(w.body, 1), en: firstParagraphs(en?.body ?? w.body, 1) },
        fullBody: (lang === "en" ? en?.body : undefined) ?? w.body, /* 展开态内联全文（Scott 同款） */
        tags: ((lang === "en" ? en?.tags : undefined) ?? w.tags ?? []) as string[],
      };
    }),
  ...
  return (
    <div className={"fb-root" + (ready ? " is-ready" : "")}>
      ...
      {loaderPhase !== "gone" && (
        <TrestleLoader onClick={() => setReady(true)} leaving={loaderPhase === "leaving"} hint={t.loaderHint} />
      )}
      ...
      <header className="fb-header">
        ...
        <nav className="fb-header-nav">
          {/* 页间往返（2026-09-20 白纸指令：简历页要能一键回文章墙）——
              在墙页=去简历，在简历页=回文章墙；文字走 pages 词表（中文=简历/首页，英文=Resume/Home） */}
          <a href={page === 0 ? "#fb-1" : "#fb-0"}
            onClick={(e) => { e.preventDefault(); go(page === 0 ? 1 : 0); }}>
            {page === 0 ? t.pages[1] : t.pages[0]}
          </a>
          ...
          <div className="fb-wallstack">
            {WALL_ITEMS.map((p, i) => {
              ...
              <Fragment key={p.group + p.no}>
                ...
                onClick={() => {
                  if (narrow) { p.slug ? navigateEntry(p.linkKind, p.slug) : setOpenIdx(openIdx === i ? null : i); return; }
                ...
                role="button" tabIndex={0} aria-expanded={narrow ? undefined : openIdx === i}>
...
function TrestleLoader({ onClick, leaving, hint }: { onClick: () => void; leaving?: boolean; hint: string }) {
  const words = LOADER_QUOTE.split(" ");
  const total = words.length;
  const attrAt = loaderWordDelay(total - 1, total) + 0.16 + 0.05;
  /* 2026-09-20 白纸指令：**不再自动进书**——引词动画放完后停住，用户点按才进入。
     （原先 setTimeout(onDone) 自动退场已删；hint 在出处行后浮现，告诉用户要点。） */
  return (
    <div className={"fb-loader" + (leaving ? " is-leaving" : "")} onClick={onClick} role="button" tabIndex={0}
      aria-label={`${LOADER_QUOTE} ${LOADER_ATTR} — ${hint}`}
      ...
      <p className="fb-loader-hint" aria-hidden="true" style={{ animationDelay: `${(attrAt + 0.35).toFixed(3)}s` }}>{hint}</p>
    </div>
  );
}
