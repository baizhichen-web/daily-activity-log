/* AI Native 第一波生成器（白纸 2026-09-13 拍板，HANDOFF §4-bis）：
   ...
   数据单源：全部事实取自 src/data/resume.ts 与 src/data/githubHeatmap.json，
...
/* 部署上线后填正式域名（如 https://baizhichen.web）；为空=链接保持根相对（llms.txt 规范允许）。
   ⚠️ 一处改全站生效：llms.txt / .md 头部链接 / JSON-LD url 全部取这里。 */
const SITE_URL = "";
...
function loadProjects() {
  ...
  const files = fs.readdirSync(CONTENT_DIR).filter((f) => f.endsWith(".yml") || f.endsWith(".yaml")).sort();
  return files.map((f) => {
    const p = parseMiniYaml(fs.readFileSync(path.join(CONTENT_DIR, f), "utf-8"));
...
function renderResumeMd() {
  const zh = IDENTITY.zh;
  const out = [];
  out.push(H("陈柏志 · 简历", 1));
  out.push(`${zh.name} — ${ROLE_LINE}\n\n`);
  ...
  out.push(H("关于"));
  ABOUT.forEach((p) => out.push(`${p}\n\n`));
  out.push(H("专业之外"));
  out.push(`${BEYOND_INTRO}\n\n`);
  BEYOND.forEach((b) => out.push(`**${b.title}** — ${b.text}\n\n`));
  out.push(H("实习经历"));
  WORK.forEach((w) => {
  ...
  out.push(H("其他项目经历"));
  OTHER_PROJECTS.forEach((r) => {
    out.push(`**${r.title}** — ${r.highlight}（${r.period}）\n\n`);
    r.lines.forEach((l) => out.push(`${l}\n\n`));
  });
  out.push(H("获奖"));
  out.push(`设计获奖\n\n`);
  AWARDS_DESIGN.forEach((a) => out.push(`- ${a}\n`));
  out.push(`\n荣誉获奖\n\n`);
  AWARDS_HONOR.forEach((a) => out.push(`- ${a}\n`));
  out.push("\n" + H("部分课程成绩"));
  COURSES.forEach((g) => {
    out.push(`**${g.group}**\n\n`);
  ...
  out.push(H("自我评价"));
  out.push(`${SELF_EVAL_BODY[0].short}\n\n${SELF_EVAL_BODY[1].short}\n\n`);
  out.push(`${SELF_EVAL_INTRO}\n\n`);
  SELF_EVAL_QUOTES.forEach((q) => out.push(`> ${q}\n\n`));
  out.push(`${SELF_EVAL_BODY[0].full}\n\n${SELF_EVAL_BODY[1].full}\n\n`);
  out.push(H("证明材料"));
  DOCS.forEach((d) => out.push(`- ${d.title}（${d.sub}）${d.href ? `：${abs(d.href)}` : "：待上传"}\n`));
  out.push("\n");
  return out.join("");
}
...
function renderContactMd() {
  const out = [];
  out.push(H("联系陈柏志", 1));
  out.push(`> 来源：[${abs("/contact.md")}](${abs("/contact.md")})（本站页面 "联系" 的 Markdown 副本）\n\n`);
  out.push(`接受实习与合作咨询，欢迎来信。\n\n`);
  out.push(`- 邮箱：202622089004@mail.bnu.edu.cn\n- 电话：+86 182 5069 8500\n- 状态：开放中（2026 Q4）\n- GitHub：https://github.com/baizhichen-web\n`);
  return out.join("");
}
...
function renderProjectMd(p) {
  ...
  out.push(`> 来源：[${abs(`/projects/${p.slug}.md`)}](${abs(`/projects/${p.slug}.md`)})（本站项目页的 Markdown 副本）\n\n`);
  const facts = [
...
function renderLlmsTxt(projects) {
  ...
  out.push(`## 页面\n\n`);
  out.push(`- [简历](${abs("/resume.md")})：教育、实习、项目经历、获奖、课程成绩、自我评价、证明材料\n`);
  out.push(`- [联系](${abs("/contact.md")})：邮箱、电话、合作状态\n`);
...
function renderLlmsFull(projects) {
  return [
    renderLlmsTxt(projects),
    "\n\n---\n\n",
...
function buildJsonLd(projects) {
  const ld = {
    ...
    "@type": "Person",
    name: IDENTITY.zh.name,
    alternateName: ["白纸上", IDENTITY.en.name],
    ...
    sameAs: ["https://github.com/baizhichen-web"],
    award: [...AWARDS_HONOR, ...AWARDS_DESIGN],
  };
...
export function buildAllFiles() {
  ...
  const contactMd = renderContactMd();
  const homeMd = renderHomeMd(projects);
  const files = {
    "llms.txt": renderLlmsTxt(projects),
    "llms-full.txt": renderLlmsFull(projects),
    "robots.txt": ROBOTS,
    "index.md": homeMd,
    "resume.md": resumeMd,
    ...
    "contact.html": pageHtml("联系陈柏志", contactMd),
    ...Object.fromEntries(projects.map((p) => [`projects/${p.slug}.md`, renderProjectMd(p)])),
    ...Object.fromEntries(projects.map((p) => [`projects/${p.slug}.html`, pageHtml(String(p.title), renderProjectMd(p))])),
  };
  const jsonLd = buildJsonLd(projects);
  return { files, jsonLd, projects };
}
...
export function aiNativePlugin() {
  ...
  const serveGenerated = (req, res, next) => {
    ...
    if (key in files) {
      res.setHeader("Content-Type", key.endsWith(".txt") ? "text/plain; charset=utf-8" : "text/markdown; charset=utf-8");
      res.end(files[key]);
      return;
    }
    next();
  ...
  return {
    ...
    transformIndexHtml() {
      ...
      return [{ tag: "script", attrs: { type: "application/ld+json" }, children: JSON.stringify(jsonLd, null, 2), injectTo: "head" }];
    },
  };
}
