...
import { projectBySlug, writingBySlug, RESUME, type ProjectBlock } from "../lib/content";
...
export function ProjectPage({ slug, kind = "project" }: { slug: string; kind?: "project" | "writing" }) {
  ...
  const mdPath = `/${mdBase}/${slug}.md`;
  const htmlPath = `/${mdBase}/${slug}.html`;
  ...
  /* 书画条目：横幅画框宽度按取景窗比例（与海报墙同式）——否则视口高矮会裁掉取景 */
  const calliAspect = (() => {
    ...
    return w > 0 && h > 0 ? c.imgAspect * (w / h) : null;
  })();
      ...
      <main className="pj-main">
        ...
        <figure className="pj-band">
          {/* 文章 = 书画（放大的文眼，与墙面展开态同一画面）；项目 = 渐变占位/真封面。
              书画条目的画框宽（72vh×取景比例）走 CSS 变量注入：窄屏由媒体查询改回 width:100%，
              否则窄屏仍按桌面公式取宽、横幅右边留白（2026-09-14 修） */}
          <div className={"pj-band-art" + (project.calligraphy ? " is-calli" : "")}
            ...
            {project.calligraphy ? (
              <CalligraphyArt
                ...
                frameAspect={calliAspect ?? undefined}
