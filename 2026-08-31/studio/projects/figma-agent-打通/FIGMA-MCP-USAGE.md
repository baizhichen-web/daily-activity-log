...
| 14 | `--allowedTools 'mcp__figma:*'` 被新版拒（08-31） | 写 `'mcp__figma__*'`（通配只许在 tool 位）或逐工具全名 |
| 15 | bailian 档 qwen3.8-max unrecognized + 小图 API 错（08-31） | Figma 任务暂用默认档；元素/截图 ≥10px；子代理无图像输入，视觉验收必须 ZCode/白纸侧做 |
| 16 | 字级变量绑不了（08-31 实测） | `setBoundVariable` 不支持 fontSize/fontWeight/lineHeight——字号类按 token 值直设+值单源；间距/圆角/填充可真绑 |
| 17 | `node.verticalAlignItems` 不存在（08-31） | HORIZONTAL 布局交叉轴对齐用 `counterAxisAlignItems`；`boundVariables` 读出为 `{type:'VARIABLE_ALIAS', id}` 对象非字符串 |
