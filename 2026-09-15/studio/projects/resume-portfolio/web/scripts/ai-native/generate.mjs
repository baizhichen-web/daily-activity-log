...
import {
  IDENTITY, ABOUT, BEYOND_INTRO, BEYOND, WORK, OTHER_PROJECTS, POSTER_WALL, EDUCATION,
  AWARDS_DESIGN, AWARDS_HONOR, COURSES, SELF_EVAL_INTRO, SELF_EVAL_QUOTES, SELF_EVAL_BODY, DOCS, ROLE_LINE, CONTACT,
...
function renderResumeMd() {
  ...
  out.push(H("教育经历"));
  EDUCATION.zh.forEach((e) => out.push(`- **${e.title}** — ${e.sub}（${e.period}）\n  - ${e.lines.join("\n  - ")}\n`));
  out.push("\n");
...
function renderHomeMd(projects) {
  ...
  out.push(`${IDENTITY.zh.lines[IDENTITY.zh.lines.length - 1]}\n\n`);
...
function buildJsonLd(projects) {
  const ld = {
    ...
    sameAs: ["https://github.com/baizhichen-web", CONTACT.xiaohongshu],
