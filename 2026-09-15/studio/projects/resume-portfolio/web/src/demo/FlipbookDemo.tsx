...
/* 取景窗 → 画框比例（白纸 Figma 稿）：画框按作品真实比例锁定 min(72vh, 宽/比例)，
   否则视口高矮会把取景裁掉（2026-09-14 实测：1200 高窗口下展开态裁掉了「魂而有知」） */
        ...
        <section className="fb-page fb-page--wall" id="fb-0">
          <div className="fb-wallstack">
            {WALL_ITEMS.map((p, i) => {
              ...
              const artW = artAspect ? `calc(var(--poster-h, 72vh) * ${artAspect.toFixed(4)})` : null;
              ...
              <Fragment key={p.group + p.no}>
                ...
                <div className="fb-poster-collapse" style={{
                    ...
                    ...(bandAspect ? { ["--poster-h"]: `min(72vh, calc(100vw / ${bandAspect.toFixed(4)}))` } : {}),
                ...
                <div className="fb-poster-art" aria-hidden
                  ...
                  style={{
                    /* 宽度：常规 100% → 27%；书画条目 → 作品比例宽（保证取景不裁，Scott 原式） */
                    width: artW ? `calc(${((1 - pr) * 100).toFixed(2)}% + ${pr.toFixed(3)} * ${artW})` : `${100 - pr * 73}%`,
                 ...
                 linear-gradient(160deg, ${mixHex(base.white, p.accent, 0.18)}, ${mixHex(base.card, p.accent, 0.34)})`,
                  ...
                  {p.calli ? (
                    <CalligraphyArt image={p.calli.image || undefined} cropFrom={p.calli.cropFrom} cropTo={p.calli.cropTo}
                      ...
                      frameAspect={bandAspect && artAspect ? bandAspect + (artAspect - bandAspect) * pr : undefined} pr={pr} />
                  ) : (
                    <>
...
function DocsFor({ section }: { section: (typeof DOCS)[number]["section"] }) {
